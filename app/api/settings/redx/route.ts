...
export async function GET() {
  ...
  const settings = await prisma.redxSettings.findUnique({
...
export async function POST(req: NextRequest) {
  ...
  await prisma.redxSettings.upsert({
    where: { userId: session.user.id },
    create: {
...
export async function DELETE() {
  ...
  await prisma.redxSettings.updateMany({
    where: { userId: session.user.id },
    data: { isConnected: false, apiKey: null, connectedAt: null },
  });
