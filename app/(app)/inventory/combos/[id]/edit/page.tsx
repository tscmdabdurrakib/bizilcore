...
export default function EditComboPage() {
  ...
  const [items, setItems] = useState<ComboItemForm[]>([{ productId: "", variantId: "", quantity: 1 }]);
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/products?all=1&withVariants=1").then(r => r.json()),
      fetch(`/api/combos/${comboId}`).then(r => r.json()),
    ]).then(([prods, combo]) => {
      setProducts(Array.isArray(prods) ? prods : []);
      if (combo?.id) {
        setName(combo.name ?? "");
        setDescription(combo.description ?? "");
        setSellPrice(String(combo.sellPrice ?? ""));
        setImageUrl(combo.imageUrl ?? "");
        setIsActive(combo.isActive ?? true);
        if (combo.items?.length) {
          setItems(combo.items.map((ci: { productId: string; variantId?: string; quantity: number }) => ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const r = await fetch(`/api/combos/${comboId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: name.trim(), description: description.trim(), sellPrice: parseFloat(sellPrice), imageUrl: imageUrl.trim(), isActive, items: validItems }),
    });
    setSubmitting(false);
    if (r.ok) {
      showToast("success", "কমবো আপডেট হয়েছে ✓");
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="font-semibold text-lg" style={{ color: S.text }}>কমবো সম্পাদনা</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="rounded-2xl border p-5" style={{ backgroundColor: S.surface, borderColor: S.border }}>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs mb-1" style={{ color: S.muted }}>বিবরণ</label>
            ...
            <div>
              <label className="block text-xs mb-1" style={{ color: S.muted }}>ছবির লিঙ্ক</label>
              <input type="url" value={imageUrl} onChange={e => setImageUrl(e.target.value)} placeholder="https://..."
                style={inp(focused === "image")} onFocus={() => setFocused("image")} onBlur={() => setFocused(null)} />
            </div>
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium" style={{ color: S.muted }}>সক্রিয়</label>
              <button type="button" onClick={() => setIsActive(v => !v)}
                className="px-3 py-1.5 rounded-full text-xs font-medium border transition-colors"
                style={{
                  backgroundColor: isActive ? "var(--c-primary-light)" : "#F3F4F6",
                  color: isActive ? S.primary : S.muted,
                  borderColor: isActive ? S.primary : S.border,
                }}>
                {isActive ? "হ্যাঁ, সক্রিয়" : "নিষ্ক্রিয়"}
              </button>
            </div>
          </div>
        </div>
