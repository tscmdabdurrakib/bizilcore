...
import { Eye, EyeOff, ChevronRight, ArrowLeft, Check } from "lucide-react";
...
export default function StoreSignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!pwOk) { setError("পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে।"); return; }
    setLoading(true);
    try {
      const res = await fetch("/api/store/customer/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ shopSlug: slug, name, email, password }),
      });
      ...
      <div className="hidden lg:flex w-1/2 bg-black flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-md">
          ...
          <p className="text-gray-400 text-base leading-relaxed mb-8">
            অ্যাকাউন্ট তৈরি করুন এবং পছন্দের পণ্য কিনুন, অর্ডার ট্র্যাক করুন।
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 lg:px-16">
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {password && (
                <div className="flex items-center gap-2 mt-2 px-1">
                  <div className={`h-1 flex-1 rounded-full ${pwOk ? (pwStrong ? "bg-green-500" : "bg-yellow-400") : "bg-red-400"}`} />
                  <span className={`text-xs font-medium ${pwOk ? (pwStrong ? "text-green-600" : "text-yellow-600") : "text-red-500"}`}>
                    {pwOk ? (pwStrong ? "শক্তিশালী" : "ঠিকআছে") : "দুর্বল"}
                  </span>
                </div>
              )}
            </div>
            ...
            >
              {loading ? (
                ...
                <>অ্যাকাউন্ট তৈরি করুন <ChevronRight size={16} /></>
              )}
            </button>

            <p className="text-xs text-gray-400 text-center leading-relaxed">
              অ্যাকাউন্ট তৈরি করলে আপনি আমাদের{" "}
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            আগেই অ্যাকাউন্ট আছে?{" "}
            <Link href={`/store/${slug}/auth/signin${redirect !== `/store/${slug}` ? `?redirect=${redirect}` : ""}`}
